...
class cost:
    ...
    def get_neighbors(self, x, y):
        neighbors = []
        if test_right(x, y, self.vertical_matrix) and (x, y) not in self.gruben:
            neighbors.append((x + 1, y, 1))
        if test_left(x, y, self.vertical_matrix) and (x, y) not in self.gruben:
            neighbors.append((x - 1, y, 0))
        if test_up(x, y, self.horizontal_matrix) and (x, y) not in self.gruben:
            neighbors.append((x, y - 1, 3))
        if test_down(x, y, self.horizontal_matrix) and (x, y) not in self.gruben:
