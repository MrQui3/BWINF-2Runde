...
Unter der Annnahme der Durchmesser der Perlen ist gleich.\\
\\
Um einen präfixfreien Code zu erzeugen, wird am besten ein Baum benutzt. Da durch diesen sicher gestellt ist, dass kein Codewort des Präfix eines anderen Codewort ist. 
Dabei ist jedes Blatt des Baums ein Codewort und jede Kante des Blatts ein Code des Codeworts. Der Baum hat mindestens so viele Blätter wie es verschiedene Buchstaben gibt.
Jeder innere Knoten hat dabei maximal so viele Kinder, wie es verschiedene Perlen gibt. Die Ebene des Blattes entspricht der Länge des Codewortes.
...
\subsection{Perlen mit gleichem Durchmessern}
Um einen möglichst kleinen präfixfreien Code zu erstellen, muss der Buchstaben der am häufigsten vorkommt ganz oben eingeordnet werden und die nächsten Buchstaben dann in absteigender Reihenfolge. 
In Abbildung \ref{fig:simple tree} hätte c die höchste Häufigkeit und dannach kommt a und b. Was von a und b die höhere Häufigkeit besitzt, ist nicht relevant, da sie beide auf der untersten Ebene sind.\\
Daher ergibt sich $p_1 \geq p_2 \geq p_3 ... \geq p_i$ und $n_1 \leq n_2 \leq n_3 ... \leq n_i$\\
Zu finden ist also ein Baum, der basierend auf den Häufigkeiten der Buchstaben, seine Blätter so auf seine Ebene so verteilt, dass die Gleichung \eqref{equation:simple_kostenberechnung} möglichst gering ist. 
Jeder innere Knoten muss dabei mindestens zwei Kinder haben, da ein innerer Knoten mit nur einem Kind die Äste des Baums verlängert ohne dem Baum einen Vorteile zu bringen.\\

Zu Beginn des Algorithmuses wird ein perfekt ausbalancierter Baum erstellt, der so viele Blätter wie Buchstaben hat (Abbildung \ref{fig:balancierter_Baum}). 
...
Um dies auszugleichen, wird das Blatt mit den höchsten Kosten eine Ebene nach oben verschoben,
indem der Elternknoten des Blattes selber zum Blatt wird und alle Kinder verliert.
Dadurch hat der Baum insgesamt zu wenig Blätter, weshalb er erweitert werden muss.
Daher werden alle möglichen neuen Blätter nach ihren Kosten überprüft und das mit den geringsten Kosten hinzugefügt. (Abbildung \ref{fig:trans_1})\\
Das nach oben verschobenene Blatt erhält den Zusatz $protected$. Das verhindert, dass es selbst weitere Kinder bekommen kann.
...
Das Problem bei unterschiedlichen Durchmessern besteht darin, dass Blätter auf derselben Ebene unterschiedliche Kosten haben können.
...
\end{figure}
Auch hier gilt: Jeder innere Knoten muss mindestens zwei Kinder haben. Bei dieser Art von Bäumen hat jedoch nicht der äußerst rechte Knoten die geringsten Kosten, sondern der oberste.
Die Kosten eines Knoten sind:
\begin{equation}
  K = K_E + K_S
\end{equation} 
wobei:\\
 - $K_E$ die Kosten des Elternknoten sind\\
 - $K_S$ die Kosten seines eigenen Knoten sind \\
...
Aus dem Text werden anschließend zwei Listen erstellt: Die erste enthält die Häufigkeit der Buchstaben in absteigender Reihenfolge, die zweite ordnet die Buchstaben entsprechend dieser Sortierung auf.
...
Zu Beginn wird zu dem Wurzelknoten, root, ein blancierter Baum erstellt.
...
\\
Neue Blätter werden über den Aufruf  
\begin{lstlisting}
