...
Jeder interne Knoten muss dabei mindestens zwei Kinder haben, da ein interner Knoten mit nur einem Kind die Äste des Baums verlängert ohne irgendwelche anderen Vorteile dem Baum zu bringen.\\
...
Dieser Vorgang wird so lange fortgesetzt, bis alle Blätter den Zusatzt $protected$ haben - also keine weitern Optimierungen mehr möglich sind.
...
Das liegt daran, dass jeder interne Knoten mindesten zwei Kinder haben muss – daher stellen diese Kosten die effektiv neu entstehenden Kosten für den Baum dar.
...
Dies ist gegeben, wenn der Baum maximal links ausgeartet ist (aBILDIUNG).
Zu Beginn der Analyse nehmen wir an jede Perle hat denselben Durchmesser.\\ 
Allgemein kann jeder Knoten maximal $n-i$ Mal verschoben werden, bis er komplett auf der linken Seite ist. Dabei ist $n$ die Anzahl an verschieden Perlen.
...
Falsche Knoten sind dabei innere Knoten, die beim ausbalancierten Baum noch nicht auf der richtigen Stelle sind, also alle bis auf die linkesten Knoten. (ABBILDUNG)
Falsche Knoten werden mit der Formel
\begin{equation}
  \text{Anzahl falscher Knoten} < \sum_{i = 0}^l n^i - l
  \label{anzahl_falscher_knoten}
\end{equation}
berechnet.
Dabei ist\\
- $l$ die maximale Anzahl der Ebenen\\
- $w$ die Anzahl der unterschiedliche Buchstaben\\

Der erste Teil der Formel beschreibt die maximale Anzahl an inneren Knoten im ausbalancierten Baum.
Dabei wird über jede Ebene iteriert und die Anzahl der inneren Knoten pro Ebene $n^i$ addiert.
Hierbei gilt $l$ als Oberste Schranke für die maximale Anzahl an Ebenen, welche wie folgt
$ log_n{w} $ 
berechnet werden kann. 
\\
\\
Der zweite Teil der Formel \eqref{anzahl_falscher_knoten} beschreibt, die schon richtig vorhanden Knoten.
In jeder Ebene gibt es sicher einen ganz linken Knoten, der schon an der richtigen Stelle ist, daher müssen diese abgezogen werden.
\\
\\
Bei unterschiedlichen Durchmesser .....
\\
\\
Daher ergibt sich allgemein eine Laufzeit von $O(n^{l+1}-l) = O(n^{\log_n{w}+1}-\log_n{w})$.
...
entwickelten Mordecai J. Golin und Günter Rote im Jahr 1998 das schnellste Verfahren, um auf eine exatke Lösung zu kommen.\\
...
Dieser Art von Algorithmus kann wegen seiner Laufzeit nicht für die vorgegebenen Beispielaufgabe verwendet werden. 
\\
Um eine polynominelle Laufzeit zu erreichen, wandeten Mordecai J. Golin, Claire Mathieu und Neal E. Young ein polynominelles approximations Verfahen an.\\
"The algotithm computes a $(1+\epsilon)$ approximate solution in time $O(n+f{\epsilon}\log^3{n})$, where $n$ is the input size."\\
