...
Der Wettbewerb wird von der BWINF gGmbH ausgerichtet und vom Bundesministerium für Bildung, Familie, Senioren, Frauen und Jugend sowie den Kultusministerien der Länder unterstützt.\cite{bwinf_about}
...
Die zweiten Runde erfolgt in Einzelarbeit, in welcher von drei angebotenen Aufgaben zwei zu lösen sind. 
Zur Endrunde werden die besten 30 Teilnehmer eingeladen, die an einem Wochenende ihr Wissen in einem Gespräch beweisen. \cite{bwinf_website}\\
...
Anschließend muss der Algorithmus auf verschiedene Testfälle angewandt werden, damit die Korrektoren die Richtigkeit und Genauigkeit beurteilen können.
...
wobei gute Ausarbeitungen Zusatzpunkte geben können und unvollständige Bearbeitungen Punktabzüge nach sich ziehen. \cite{bwinf_aufgabe}\\
...
Zuerst erkläre ich jeweils kurz die Aufgabestellung. 
Danach beschreibe ich meine theorietische Überlegung und zeige im Anschluss, wie ich sie praktisch in Python3 umgesetzt habe.
...
\enquote{Ein Code C heißt Prafix-Code (irreduzibler Code), wenn kein Codewort aus C Prafix eines anderen Codeworts von C ist.}\cite{schulz2018codierungstheorie}
...
Im Gegensatz dazu ist der Code \{0, 01, 11\} nicht präfixfrei ist, weil "'0"' der Präfix von "'01"' ist.
...
\\
Um einen präfixfreien Code zu erzeugen, wird überlicherweise ein Baum verwendet.
...
Jeder innere Knoten kann dabei maximal so viele Kinder haben, wie es verschiedene Perlen gibt. 
...
Daher kann das Problem auf ein Baum reduziert werden.\\
\begin{figure}[h]
  \centering
...
Jeder innere Knoten muss dabei mindestens zwei Kinder haben, da ein innerer Knoten mit nur einem Kind die Zweige des Baums verlängert (höheres $n$) ohne einen Vorteile zu bringen.\\
\\
...
Durch die Neuzuweisung ist sicher gestellt, dass immer die äußerst linken Blätter, die Buchstaben mit den geringsten Häufigkeiten kriegen und rechts die höchsten.\\
Der Vorgang, ein Blatt nach oben zu schieben und unten neue Blätter hinzufügen, wird so lange fortgesetzt, bis keine neuen Blätter mehr eingefügt werden können, da alle Blätter als $protected$ gelten.
...
\subsubsection{Erweiterung auf Symbolen mit unterschiedlichen Kosten}
...
Daher entsprechen die Kosten nicht mehr der Wert der Ebene.
...
\\
 Stellt man nun Bäume nicht nach ihrer Ebene, sondern nach ihren Kosten dar, erhält man einen \textit{lopsided tree}\cite{golin1998prefixfree} (Abbildung \ref{fig:lopsided tree}).\\
 \begin{figure}[h]
  \centering
...
\end{figure}\\
In diesem Fall gilt ebenfalls, jeder innere Knoten muss mindestens zwei Kinder haben. 
...
Das Baum ist über die Klasse \textit{TreeNode} rekursiv implementieren. Jeder Knoten entspricht einem TreeNode-Objekt.
\begin{figure}[H]
  \centering
...
\end{lstlisting}
Die Methode $create\_tree$ bekommt über den Aufruf $get\_all\_possible\_costs()$ eine Liste an allen möglichen neuen Blätter, die der Baum als nächstes hinzufügen kann.
...
Dazu wird der Iterationsschritt über den Aufruf $improve\_tree(root)$ wiederholt und jeweils mit der Methode $calculate\_tree(root)$ die neuen Kosten des Baums berechnet.
...
Die Durchmesser sind hierbei unerheblich.
...
Falsche Knoten sind dabei innere Knoten, die beim ausbalancierten Baum noch nicht auf der Stelle des ausgearteten Baums sind, also alle bis auf die äußerst linken Knoten. (Abbildung \ref{fig:makiert_baum})
...
\end{equation}
berechnet werden.\cite{schulz2018codierungstheorie}
...
\end{figure}
Abbildung \ref{fig:iteration_tree} zeigt wie die Länge der codierten Nachricht der Testfall $schmuck9$ sich über die Iterationen hinweg verändert.
...
entwickelte bereits Richard Karp 1961\cite{karp1961minimum} eine Lösung. Diese wurde 1998 von Mordecai J. Golin und Günter Rote verbessert und ist bis heute das schnellste Verfahren, um auf eine exakte Lösung zu kommen.\\
...
Diese Art von Algorithmus kann wegen seiner langen Laufzeit nicht für die vorgegebenen Beispielaufgabe verwendet werden.
\\
\\
...
Dabei ist es zentral, dass die Wahrscheinlichkeitsverteilung der zu kodierenden Codewörter möglichst in großen Gruppen mit gleich großer Wahrscheinlichkeit eingeordnet werden können. \cite{golin2012huffman}
Zum Beispiel in Testfall 9 ist dies nicht gegeben, weshalb das Bestimmen des Vorbaums über $9^{24}$ Schritte benötigt.
...
Der erste Anssatz ist ein Greedy-Algorithmus basierend auf Huffman.
...
Trotzdem entschied ich mich dagegen eines dieser Verfahren zu implementieren, vor allem aufgrund deren hohen Laufzeit, 
...
Für meine gutes ErgebniArbeit erhielt ich daher eine Pluspunkt und erreichte insgesamt 21 Punkte für diese Aufgabe.
...
Die dritte Person gibt jeweils beiden immer dieselbe nächste Schrittanweisung. 
...
Die ursprüngliche Aufgabe enthält normale Labyrinthe. Im Spezialfall werden die Labyrinth um Gruben erweitert.
...
es sei denn, beide Labrinthe schlagen denselben Schritt vor.
...
Denn wichtig ist vorallem, dass ihre Kosten (Gleichung \eqref{weg_kosten}) gering sind.
...
Wenn einer der beiden Personen eine Grube berührt, wird seine Position auf die jeweilige Startposition zurückgesetzt.
...
Das würd solange wiederholt, bis der $stack$ leer ist.
...
\\
Zu Beginn wird ein $Solving$-Objekt initalisiert und eine leere Liste $moves\_stack$ angelegt, in der alle aktuellen Wege gespeichert werden.
...
Daraufhin werden die neuen zwei Positionspaare berechnet und überprüft, ob die beiden Positionspaare bereits besucht wurden.\\
Zum Schluss wird für jeden möglichen Zug ein $Move$-Objekt erstellt und zusammen in einer Liste übergeben.
Falls einer der neuen Positionspaare bereits besucht wurde oder bereits im Ziel ist, wird der entsprechende Zug nicht übergeben (siehe Abbildung \ref{fig:neighbours_cost}).
\\
\\


\subsection{Laufzeitanalyse}
Von jedem Positionspaar aus existieren im schlimmsten Fall zwei mögliche Schritte, entweder in Richtung des Ziels von Labyrinth 1 oder in Richtung des Ziels von Labyrinth 2.
Das führt zu insgesammt $2^{(nm)^2}$ verschiedenen Möglichkeiten an Schritten.
...
Wenn die Steigung positiv ist, wird für ein größeren Wert von $l$ ein schlechterer Pfad gefunden.
...
\begin{figure}[h]
  ...
  \caption{maximale Iterationen gegenüber tatsächliche Iterationen aufgetragen gegenüber der Begrenzung}
...
\\
\\
Die Anzahl der Iterationen stehen in einem liearem Zusammenhang mit den Werten von $l$,
...
Diese Variante wird jedoch in der Musterlösung aufgrund ihrer hohe Laufzeit als nicht praktikabel eingestuft.\\
Die zweite Stratie basiert auf dem A-Star Algorithmus mit Priorty Queue. 
...
Insgesamt lagen meine Ergebnisse, aus geschlossen von Testfall 8, bei 1,1\% Abweichung vom Optimalwert.\cite{bwinf_loesung}
Bei Testfall 8 fiel mein Ergebnis etwa 3-mal so groß aus, weshalb mir 2 Puntke im Kriterium "'Verfahren mit guten Ergebnissen"' abgezogen wurden.
Außerdem war meine Überlegung zur Laufzeit des Verfahren nicht ausreichend detailliert, was zu einem weiteren Punktabzug führte.\\
Insgesamt erhielt ich für die Aufgabe 17 von 20 Punkten.

\section{Fazit}
1. kurz zusammenfassen was ich gemacht haben in aufgabe1
2. was für ergebnisse ich erzielt habe damit; wie viel punkte
3. kurz zusammenfassen was ich gemacht haben in aufgabe2
4. was für ergebnisse ich erzielt habe damit; wie viel punkte
welcher paltz ich wurde
was mit das gebracht habe
warum ich dem wettbewerb gut finde
...
Aufgrund des begrenzten Umfangs der Arbeit konnte ich keinen Beweis für die NP-Schwere erbrignen und eine präzisere Laufzeitanalyse durchführen.
Dennoch konnte ich durch Vergleiche mit der Musterlösung Erkenntnisse über die Effizienz verschiedener Algorithmusstrategien gewinnen\\
Insgesamt konnte ich mit 38 von 40 Punkten den zweiten Platz erreichen und gehörte damit zu den 60 besten Teilnehmenden von über 250.
...
Durch diese Arbeit lernt ich Laufzeitanlysen selbstständig durchzuführen und mit wissenschaftlichen Texten zu arbeiten.
Der Bwinf erweist sich insgesamt als gute Möglichkeit, Schüler an wissenschaftliches Arbeiten und algorithmisches Denken heranzuführen.


Der Bwinf eigenet sich gut 
udn sollte gefördert werden
...
Die vorgegebenen Testfälle können in Quelle 7 nachgeschaut werden und die optimalen Ergebnisse können in Quelle 6 eingesehen werden.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|c|}
  \hline
  \textbf{Testfall} & \textbf{meine Gesamtkosten} & \textbf{optimalen Ergebnis} \\ \hline
