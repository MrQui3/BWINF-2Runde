...
Der Wettbewerb wird von der BWINF gGmbH ausgerichtet und von dem Bundesministerium für Bildung, Familie, Senioren, Frauen und Jugend sowie den Kultusministerien unterstützt.[11]
...
\end{figure}
Abbildung \ref{fig:iteration_tree} zeigt wie die Länge der codierten Nachricht der Testcase $schmuck9$ sich über die Iterationen hinweg verändert.
...
Zum Beispiel in Testcase 9 ist dies nicht gegeben, weshalb das Bestimmen des Vorbaums über $9^{24}$ Schritte benötigt.
...
Beide Labyrinthe haben die selben rechteckigen Maße (n, m).
Zwei Personen starteten jeweils in ihrem Labyrinth bei der Position (0, 0) und müssen das Ziel (n, m) erreichen.
...
in welcher ersten Teilaufgabe es sich um Labyrinthe ohne Grube handelt und in der zweiten Teilaufgabe um Labyrinth mit Grube.
Fällt ein Spieler in eine Grube, wird seine Position auf die Startposition (0, 0) zurückgesetzt.[5]
Ich bearbeite zuerst die Variante ohne die Gruben und erweitere sie anschließend auf Labyrinthe mit Gruben.
\subsection{Lösungsidee}
...
Die gespeicherte Richtung zeigt den optimalen Folgeschritt zum Ziel und ist wie in Tabelle \ref{tab:richtungscodierung} dargestellt.
\begin{table}[h]
  \centering
  \begin{tabular}{|c|l|}
      \hline
...
Ein Labyrinth mit dazugehöriger Matrix könnte so wie in Abbildung \ref{fig:lybnmatrix} aussehen.
\begin{figure}[H]
  \centering
...
Den nächsten Schritt ermittelt er, indem er in den Matrizen prüft, welchen Schritt die beiden Labyrinthe für ihre jeweilige Position vorschlagen.
...
Dabei ist 2 der maximal erreichbare Wert. Dieser Fall ist nur bei identischen Labyrinthen gegeben.
\\
...
Die Kosten der Grubenfelder entsprechen den Startkosten.
...
\end{figure}
\\
Abbildung \ref{fig:schritte_1} zeigt die Anzahl der Schritte des gefundenen Lösungswegs in Abhängigkeit vom Parameter $l$ für Testcase 4.
\\
\\
