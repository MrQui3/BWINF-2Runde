...
in der für jede Position ein Kostenwert und eine Richtung gespeichert wird. Die gespeicherte Richtung zeigt den optimalen Folgeschritt zum Ziel und ist dargestellt als
\begin{table}[h]
  \centering
  \begin{tabular}{|c|l|}
      \hline
...
Der Bruch beschreibt also die durchschnittliche Einsparung an Kosten pro Schritt
...
Jeder Eintrag in der Liste besitzt vier Werte: der x-Position, der y-Position, den aktuell Kosten sowie der entgegengesetzen Richtung, aus welcher das Feld betreten wurde. 
...
\end{lstlisting}
Zu Beginn werden die beiden Positionen aus dem Attribut $positions$ des übergebenen $move$-Objekt genommen.
...
Der hier vorgestellte Ansatz garantiert daher nicht unbedingt den optimalen Lösungsweg, liefter jedoch in akzeptabler Laufzeit eine gute Annäherung.
Der Algorithmus ähnelt einer Tiefensuche, da er einige Wege tief verfolgt, während andere nur kurz betrauchtet werden.
...
Ebenso erkennbar ist das es nicht streng monoton fallend ist. 
...
\end{figure}
Abbildung \ref{fig:laufzeit_1} stellt die Anzahl an Iterationen dar, die zur Bestimmung des Lösungsweg benötigt werden in Abhängigkeit vonm Parameter $l$.
\\
...
während die Anzahl der Schritte bei größreren $l$-Werten gegen das optimale Ergebnis konvergieren.
Je größer der Wert von $l$, desto mehr Interation werden durchgeführt - und desto besser wird das Ergebnis.

Die Genauigkeit lässt sich anhand der Parameters $l$ jedoch nicht allgemein bestimmen, da sie stark von der Strucktur jeweiligen Labyrinthe abhängt.
...
Alle Schritt des Wegs werden wie in der Tabelle \ref{tab:richtungscodierung} codediert.
...
Weg: [0, 0, 3, 0, 2, 0, 0, 0, 3, 1, .... 2, 2, 1, 3, 3, 1, 1, 3, 0, 0, 0, 0, 0]
...
\end{figure}
Länge des Wegs: 1844\\
Weg: [0, 0, 0, 3, 3, 3, 3, 3, ..., 0, 0, 3, 0, 0, 2, 2, 0, 0, 0, 3, 3]
...
\end{figure}
Länge des Wegs: 1433\\
Weg:[3, 0, 0, 3, 3, 3, 3, 0, 3, 3, ..., 0, 3, 3, 3, 0, 2, 0, 3, 3, 0]
...
def main():
   width, height, data = read_data_from_file('labyrinthe_.txt')
