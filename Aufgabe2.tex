...
Von jeder Position aus existieren zwei mögliche Schritte. Entweder in Richtung ded Ziel von Labyrinth 1 oder in Richtung des Ziel von Labyrinth 2.
Das führt zu insgesammt $2^{\text{maximale Anzahl an Schritten}}$ verschiedenen Möglichkeiten.
Aufgrund dieser großen Anzahl an Möglichkeiten untersucht der Algorithmus nicht alle Pfade vollständig, sondern verwendet Heuristik zur Abschätzung.
...
Auffällig ist, dass der Graph in Abbildung \ref{fig:schritte_1} gegen den optimalen Weg abfallend konvergiert.
Der Graph in Abbildung \ref{fig:laufzeit_1} hingegen stark steigt.
\\
...
Daher ergibt sich die Gleichung \eqref{max_iterationen}, was eine liniare Laufzeit von $o(l)$ ergibt.

\clearpage
Praktisch gesehen entspricht ist die Laufzeit, aber sehr viel kleiner als die maximale Laufzeit.
\begin{figure}[h]
  \centering
...
\\
Auffällig ist auch der kleine Hügel in Abbildung \ref{fig:schritte_1}. 
Hier wird efür ein größeren Wert von $l$ ein schlechterer Pfad gefunden.
...
\\
\\
Anhand des Parameters $l$ lässt sich die Genauigkeit jedoch nicht allgemein bestimmen, da sie stark vom jeweiligen Labyrinthe abhängt. 
